//! A text editor drawing a terminal-style block caret.
//!
//! This is `iced_widget::text_editor` from `iced_widget` 0.14.2, vendored so
//! that the caret can be drawn as a filled block one character wide in a colour
//! the theme chooses. Iced draws its caret as a one-pixel line in the text
//! colour, and neither the width nor the colour is reachable through
//! [`Style`], so there is no way to do this from outside the widget.
//!
//! Everything here is upstream code except for three changes, each marked with
//! a `LANTERN:` comment:
//!
//! 1. the imports below, rewritten from `iced_widget`'s internal `crate::core`
//!    paths onto the public `iced` ones,
//! 2. [`Style`] carries a `caret` colour, and
//! 3. the caret is drawn as a block one character wide, sized by
//!    [`super::caret`], which recolours the character it covers.
//!
//! Upstream's own conventions are kept as they are, so that re-syncing against
//! a later `iced_widget` stays a matter of re-applying four small diffs rather
//! than a re-reading of the whole file. That includes the `expect` calls and
//! the doc comments Lantern would otherwise write differently. Re-syncing is:
//! copy the file over, redo the imports, re-apply the three changes below, and
//! run `cargo fmt` — the layout here is this workspace's rustfmt, not
//! `iced_widget`'s narrower one.
//!
//! The blink is upstream's: [`Focus::CURSOR_BLINK_INTERVAL_MILLIS`], 500ms,
//! redrawn by the widget asking for a frame at each interval boundary.

// LANTERN: upstream code carries four things Lantern's own conventions would
// not, and they are named rather than blanketed so that a re-sync bringing in
// anything else is still reported: `State::is_focused`, which the application
// never calls; the `Position` re-export, which it never names; `cfg`s naming
// `iced_widget`'s features rather than Lantern's, so `highlight` and `class`
// are compiled out, neither being reachable here; and the highlighter's
// formatting function, whose type upstream spells out in full.
#![allow(dead_code, unexpected_cfgs, unused_imports, clippy::type_complexity)]

// LANTERN: imports rewritten from `iced_widget`'s `crate::core` onto `iced`.
    ...
    fn draw(
        ...
        if let Some(focus) = state.focus.as_ref() {
            match internal.editor.selection() {
                // LANTERN: a block one character wide in the theme's caret
                // colour, rather than a one-pixel line in the colour of the
                // text.
                Selection::Caret(position) if focus.is_cursor_visible() => {
                    let text_size = self.text_size.unwrap_or_else(|| renderer.default_size());

                    // The renderer draws every quad in a layer before any of
                    // its text, so this block lands behind the line and the
                    // editor's own text pass draws the character on top of it.
                    // Drawing the character a second time here would be a
                    // second rasterisation of the same glyph at a different
                    // subpixel offset, which leaves the first one showing
                    // around the edges of the second as a coloured fringe.
                    let cursor = Rectangle::new(
                        position + translation,
                        Size::new(
                            super::caret::width::<Renderer>(font, text_size, self.line_height),
                    ...
                    if let Some(clipped_cursor) = text_bounds.intersection(&cursor) {
                        ...
                        // The whole line is drawn again in the caret's text
                        // colour and clipped to the caret, which recolours the
                        // one character inside it. This is the same editor at
                        // the same origin as the pass above, so its glyphs land
                        // on exactly the pixels that pass drew. Laying the
                        // character out separately instead would rasterise it
                        // at its own subpixel offset, and the first drawing
                        // would show around the second as a coloured fringe.
                        renderer.fill_editor(
                            &internal.editor,
                            text_bounds.position(),
                            style.caret_text,
...
pub struct Style {
    ...
    pub selection: Color,
    /// LANTERN: the [`Color`] of the block caret.
    pub caret: Color,
    /// LANTERN: the [`Color`] of the character the block caret covers.
    pub caret_text: Color,
}
...
pub fn default(theme: &Theme, status: Status) -> Style {
    ...
    let active = Style {
        ...
        // LANTERN: the accent colour reads as a caret against body text, and
        // the palette pairs it with a colour legible on top of it.
        caret: palette.primary.base.color,
        caret_text: palette.primary.base.text,
    };
